...
type Wallet struct {
	...
	seed      []byte
	url       accounts.URL
	paths     map[common.Address]DerivationPath
...
// of tracked accounts.
func (w *Wallet) Derive(path DerivationPath, pin bool) (accounts.Account, error) {
...
func (w *Wallet) SelfDerive(base DerivationPath, chain hpb.ChainStateReader) {
...
func (w *Wallet) SignTx(account accounts.Account, tx *types.Transaction, chainID *big.Int) (*types.Transaction, error) {
	...
	signedTx, err := types.SignTx(tx, types.NewBoeSigner(config.MainnetChainConfig.ChainId), privateKey)
	if err != nil {
		return nil, err
	}

	msg, err := signedTx.AsMessage(types.NewBoeSigner(config.MainnetChainConfig.ChainId))
	if err != nil {
		return nil, err
	}
...
func (w *Wallet) deriveAddress(path DerivationPath) (common.Address, error) {
